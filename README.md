...
[![Build Status](https://travis-ci.org/VahidIrn/NeoEloquent.svg?branch=master)](https://travis-ci.org/VahidIrn/NeoEloquent)
...
```php
'VahidIrn\NeoEloquent\NeoEloquentServiceProvider',
```
...
```php
$app->register('VahidIrn\NeoEloquent\NeoEloquentServiceProvider');
```
...
```php
namespace VahidIrn\Cms;

class Admin extends NeoEloquent { }
```

The generated label from that relationship will be `VahidIrnCmsAdmin`, this is necessary to make sure
that labels do not clash in cases where we introduce another  `Admin` instance like
`VahidIrn\Blog\Admin` then things gets messy with `:Admin` in the database.
...
To enable soft deleting you'll need to `use VahidIrn\NeoEloquent\Eloquent\SoftDeletes`
...
```php
use VahidIrn\NeoEloquent\Eloquent\SoftDeletes;
...
To enable soft deleting you'll need to `use VahidIrn\NeoEloquent\Eloquent\SoftDeletingTrait`
...
```php
use VahidIrn\NeoEloquent\Eloquent\SoftDeletingTrait;
...
// $relation will be VahidIrn\NeoEloquent\Eloquent\Edges\EdgeIn
...
$relation = $location->associate($user); // VahidIrn\NeoEloquent\Eloquent\Edges\EdgeIn
...
- `use VahidIrn\NeoEloquent\Schema\Blueprint;`
- `use VahidIrn\NeoEloquent\Migrations\Migration;`
...
## Changelog
Check the [Releases](https://github.com/VahidIrn/NeoEloquent/releases) for details.
